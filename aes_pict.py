...
import numpy as np
import pickle
from M16_show_pictures import Print_pic
...
class AESModeOfOperation(object):
    ...
    def convertA(self, string, start, end, mode):
    ...
    def encrypt(self, stringIn, mode, key, size, IV):
        ...
        if stringIn != None:
            for j in range(int(math.ceil(float(len(stringIn))/16))):
                ...
                plaintext = self.convertA(stringIn, start, end, mode)
                ...
                if mode == self.modeOfOperation["CFB"]:
                    ...
                    output = self.aes.encrypt(IVV, key, size)
                    for i in range(16):
                        if len(plaintext)-1 < i:
                            ciphertext[i] = 0 ^ output[i]
                        elif len(output)-1 < i:
                ...
                if mode == self.modeOfOperation["CFB"]:
                    ...
                    for i in range(16):
                        if len(ciphertext)-1 < i:
                            plaintext[i] = 0 ^ output[i]
                        elif len(output)-1 < i:
                            plaintext[i] = ciphertext[i] ^ 0
                        elif len(ciphertext)-1 < i and len(output) < i:
                            plaintext[i] = 0 ^ 0
                        else:
                            plaintext[i] = ciphertext[i] ^ output[i]
                    for k in range(end-start):
                        chrOut.append(plaintext[k])
                    IV[-1] += 1
                    pind = 3
                    while(IV[12+pind] == 256):
                        IV[12+pind] = 0
                        IV[12+pind-1] += 1
                        pind -= 1
        return chrOut
...
file = open(f'res/CTR/AES/Full_text_C', 'wb')
pickle.dump(CC, file)
file.close()
